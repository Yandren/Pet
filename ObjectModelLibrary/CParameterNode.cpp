...
int32_t CParameterNode::GetInt() const
{
	...
	mLastResult = EPR_OK;
	return *(reinterpret_cast<const int32_t*>(&mData));
}
...
CHash	CParameterNode::GetHash(void) const
{
	...
	uint32_t hashVal = *reinterpret_cast<const uint32_t *>(&mData);
...
int32_t CParameterNode::GetInt(CHash name) const
{
	CParameterNode *child = GetChild(name);
	if (child == NULL)
	{
